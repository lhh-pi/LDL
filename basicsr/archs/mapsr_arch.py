...
class LCB(nn.Module):
    def __init__(self, num_feat, scale_factor=30):
        super(LCB, self).__init__()

        self.lcb = nn.Sequential(
            nn.Conv2d(num_feat, num_feat // scale_factor, 3, 1, 1),
            nn.ReLU(inplace=True),
            nn.Conv2d(num_feat // scale_factor, num_feat, 3, 1, 1),
            nn.ReLU(inplace=True)
        )
...
class STLC(nn.Module):
    r""" Swin Transformer with Local CNN block.
    ...
    def __init__(self,
        ...
        self.conv_scale = conv_scale  # conv_scale: 融合系数 alpha = 1.0, 0.1, 0.01
        self.conv_block = LCB(num_feat=dim, scale_factor=scale_factor)
    ...
    def forward(self, x, x_size):
        ...
        # FFN
        x = shortcut + self.drop_path(attn_x) + conv_x * self.conv_scale  # self.conv_scale 融合系数 alpha = 1.0, 0.1, 0.01
...
class BasicLayer(nn.Module):
    """ A basic STLC layer for one stage.
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False, alpha=0.1):
        ...
        self.blocks = nn.ModuleList([
            STLC(dim=dim, input_resolution=input_resolution,
...
class RSTB(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 img_size=224, patch_size=4, resi_connection='1conv', alpha=0.1):
